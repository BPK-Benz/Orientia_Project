...
def calculation_week(n, func, set1):
    ...
    x7 = [i/15 for i in set1['Bac/Inf'].tolist()]
    
    
    popt,pcov = curve_fit(func,(x1,x2,x3, x4,x5,x6, x7),y, maxfev=1000000) # This line is difference
    y_pred = np.round(func((x1,x2,x3, x4,x5,x6, x7), *popt),4)
...
def calculation2_week_value(func, popt, set1):
    ...
    y_pred = func((x1,x2,x3, x4,x5,x6, x7), *popt)
    return y_pred
...
for a in parent_map:
    ...
    net_results.to_csv(path+'Adaptive_RegressionModel/10_Replicate_net_results_model_Trans_'+function_name+'.csv')
...
for file in glob.glob(path+"Adaptive_RegressionModel/10_Replicate_net_results_model_Trans*"):
...
dfff.columns = [j if i == 0 else j.split('Trans_')[1] for i,j in enumerate([b] + name)]
...
for order, m in enumerate(summary['Math_model']):
    k = pd.read_csv(glob.glob(path+"Adaptive_RegressionModel/10_Replicate_net_results_model_Trans*"+m+'*')[0]).iloc[:,-10:]
    ...
    kk = pd.read_csv(glob.glob(path+"Adaptive_RegressionModel/10_Replicate_net_results_model_Trans*" + m + '*')[0]).iloc[:,1:11]
...
for order, batch in enumerate(['Batch_0'+str(i) if i < 10 else 'Batch_'+str(i) for i in range(1,11)]):
    ...
    df_gene['Scramble_Trans (Model)'] = prediction.tolist()
...
for order, batch in enumerate(['Batch_0'+str(i) if i < 10 else 'Batch_'+str(i) for i in range(1,11)]):
    ...
    df_ss['Scramble_Trans (Model)'] = prediction.tolist()
...
sns.scatterplot(data=model_ss_df, x='Nuc/Inf',y='Scramble_Trans (Model)', hue='Batch_New', palette=colors)
plt.ylim(0,10)
plt.xlim(0,10)
x = range(0
          ,10)
...
for ii, g in enumerate(set(model_df['Gene_Symbol'].tolist())):
    ss = model_df[(model_df['Gene_Symbol'] == g)]['Scramble_Trans (Model)'].tolist()
    gg = model_df[(model_df['Gene_Symbol'] == g)]['Nuc/Inf'].tolist()
    z,n,mg,sdg, ms, sds = ssmd(ss,gg)
    ssmdvalue = ssmdvalue + [[g,z,n,mg,sdg, ms, sds]]

ssmd_df_b = pd.DataFrame(ssmdvalue, columns=['Gene_Symbol','ssmd_Trans','Interpret_Trans','Mean_gene','STD_gene','Mean_scramble','STD_scramble']).set_index('Gene_Symbol')
...
Z_volcano.to_csv(path+'Adaptive_analysis_result_Trans_ssmd_name_May_2021.csv')
Z_volcano = Z_volcano.reset_index().sort_values(by='Plate')


qs = Z_volcano.groupby('Plate')['ssmd_Trans'].quantile([0.25,0.75])
qs = qs.unstack().reset_index()
qs.columns = ["Plate", "q1", "q3"]
qs['IQR'] = qs['q3']-qs['q1']
qs['Median'] = pd.DataFrame(Z_volcano.groupby('Plate')['ssmd_Trans'].median())['ssmd_Trans'].tolist()
qs['Batch'] = [map_dict[f] for f in qs['Plate']]
qs['Minimum'] = Z_volcano.groupby('Plate')['ssmd_Trans'].min().tolist()
qs['Maximum'] = Z_volcano.groupby('Plate')['ssmd_Trans'].max().tolist()
qs = qs.set_index('Plate')

######## plot graph to show an example of outlier detection 
fig, ax = plt.subplots(figsize=(8,14))
g = sns.boxplot(y='Plate', x='ssmd_Trans', data=Z_volcano.sort_values(by='Plate') ,dodge=False, hue='Batch', palette=colors)
number = 1.2
...
plt.title('Bacteria Trans (Adaptive): Median +/- '+str(number)+'*IQR ', fontsize=12, fontweight='bold')
# plt.show()
...
for b,nb in zip([-1,1],['(Inhibit)','(Enhance)']):
    ...
    for t in range(1000,700,-400):
        ...
        for n in np.arange(0.005, 2.01, 0.001).tolist():
            ...
            if b < 0:
                Inhibit_gene = Z_volcano[Z_volcano['ssmd_Trans'] < b*Z_volcano['Threshold']]
            else:
                Inhibit_gene = Z_volcano[Z_volcano['ssmd_Trans'] > b*Z_volcano['Threshold']]
    ...
    for e, n in zip(threshold['Expected_Number_gene'] , threshold['Constant']):
        ...
        Z_volcano['Threshold'] = [qs.loc[p,'Threshold'] for p in Z_volcano['Plate']]
        if b < 0:
            Inhibit_gene = Z_volcano[Z_volcano['ssmd_Trans'] < -1*Z_volcano['Threshold']]
        else:
            Inhibit_gene = Z_volcano[Z_volcano['ssmd_Trans'] > 1*Z_volcano['Threshold']]
        
        hit = Inhibit_gene
        hit.to_csv(path+'Adap_Trans_Hit_genes_'+str(e)+'_'+nb+'.csv')
        fig,ax = plt.subplots(figsize=(10,12))
        sns.heatmap(hit.pivot_table(index='Plate', columns='Batch', values='ssmd_Trans', aggfunc='count',  
